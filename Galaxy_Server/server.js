...
server.use(async(r,rs,nxt)=>{
  ...
  let Is_non_authenticatable_request = ["signin","signup","forgetpassword","verifyotp",
    "changepassword","defualt.png","security",".txt",".js",".ttf",".css",".apk","p"].every((v,k)=>!r.url.includes(v))
  console.log(r.url)
...
server.use(async(rq,rs,next)=>{
  ...
  let Is_non_securable_requests = ["security","deleteaccount","postanalytics","homeposts"].every((v,k)=>rq.url.includes(v))
