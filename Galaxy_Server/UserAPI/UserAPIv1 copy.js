                    ...
                    try{
                        await rs.userprofiles.insertOne({
                            ...
                            settings:{
                                ...
                                profileImage:"http://localhost:7000/default.png"
                            },
...
async function Signin(rq,rs){
    ...
    if(gid != null){
        if(gid.password == rq.body.password){
            let token = jwt.sign({
                gid:rq.body.gid
                ...
                if(er){
                    ...
                    let UploadProfileImage = async function(){
                            ...
                            if(user.settings.profileImage.includes("default")){
                                ...
                                {$set:{
                                "settings.profileImage":`http://localhost:${process.env.PORT}/${user.GID+".txt"}`
                                }})).modifiedCount == 1
...
async function getPosts(rq,rs){
    ...
    if(mainuser != null){
        if(user != null){
            let posts = await rs.userposts.find({owner:user.GID},{projection:{date:0,_id:0}}).toArray()
    ...
    if(user != null){
        ...
        if(post != null){
            ...
            if(postusrprofile != null){
                ...
                let replys  = (await rs.userreplys.find({postid:post.postid},{projection:{_id:0,date:0,postid:0}}).toArray())
