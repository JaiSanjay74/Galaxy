const server_origin = "http://localhost:7000" //"https://yak-upright-urgently.ngrok-free.app"
...
export async function UpdateAccount(token,credential) {  // credential should be object data type
    let update = async(cred,data)=>{
        ...
        let rdata = await(await fetch(server_origin + "/api/updateaccount",{
        method:"PUT",
        headers:{
            "Content-Type":"application/json",
            Authorization:"Bearer "+token
    ...
    // Decryption Server Data (Response)
    let decrypted = await DecryptAESEncryptedData(rdata)
    return JSON.parse(decrypted).data
    }
