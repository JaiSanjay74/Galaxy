...
onMounted(async()=>{
    ...
    if(datum.type === "private"){
        ...
        else if(serverdata === "UNKNOWN_ACCOUNT"){
            msg({msg:"Unkown user account !",error:true,manual:false,type:"show"})
        }
        else{
        ...
        let serverdata = await getPosts(localStorage.getItem("GToken"),datum.type,datum.gid)
        if(serverdata === "USER_NOT_FOUND"){
           msg({msg:"User not found !",error:true,manual:false,type:"show"})
        }
        else if(serverdata === "UNKOWN_ACCOUNT"){
...
onUnmounted(()=>{
    if(datum.type === "home"){
        sessionStorage.removeItem("recpst")
    }
})
</script>
