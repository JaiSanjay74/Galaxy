...
<template>
 ...
 <div class="app" v-if="screen == 'application'">
   <div class="a1">
    ...
    <nav>
        <div>
        <section :class="(window == 'posts' && postpurpose == 'home') ? 'scrn':''" @click="()=>{window = 'posts';postpurpose = 'home'}">
            <i class="fa-solid fa-house"></i>
            <b>Home</b>
        </section>
        <section :class="(window == 'profilelist' && profilelistpurpose == 'followers') ? 'scrn':''" @click="()=>{window = 'profilelist';profilelistpurpose = 'followers'}">
            <i class="fa-solid fa-user-plus"></i>
            <b>Followers</b>
        </section>
        <section :class="(window == 'profilelist' && profilelistpurpose == 'followings') ? 'scrn':''" @click="()=>{window = 'profilelist';profilelistpurpose = 'followings'}">
            <i class="fa-solid fa-person-walking"></i>
            <b>Following</b>
        </section>
        <section :class="(window == 'posts' && postpurpose == 'private') ? 'scrn':''" @click=" window = 'posts';postpurpose = 'private'">
            <i class="fa-solid fa-signs-post"></i>
            <b>My posts</b>
        </section>
        <section :class="(window == 'profile' && postpurpose == 'private') ? 'scrn':''" @click="()=>{window = 'profile';postpurpose = 'private'}">
...
<div class="home" v-else>
   <div class="mmsg" v-if="homemsg != null">
    ...
    <Login v-if="window == 'login'" :isFromApp="isLogin.v" @reload="logged" @app="screen = 'application'" @msg="ShowHomeMessage"/>
    <Signup v-if="window == 'signup'"  @msg="ShowHomeMessage"/>

    <b id="download" v-if="!IsWebView && window === null">Galaxy app for android - <a href="https://yak-upright-urgently.ngrok-free.app/Galaxy.apk">Download</a></b>
  </div>
...
<script setup>
import { provide, ref ,onMounted, onUpdated, reactive} from "vue";
...
let logged = async()=>{
    /* if webview */
        IsWebView.value = navigator.userAgent.includes("Gapp")
    /* -----------*/
    ...
    if(IsAlreadyLogged){
        ...
        if(cred === "EXPIRED"){
            ...
            //let imgBase64 = await (await fetch(cred.settings.profileImage)).text()
            ucred.value.gid = cred.GID
            ucred.value.name = cred.name
            ucred.value.img = cred.settings.profileImage
            console.log(ucred.value.img)
